...
public static class ControllerExtensions
{
	...
	public static string? ExtractWhereQuery(this ControllerBase _, string query, string? defaultValue = null)
	{
	...
	public static void ExtractPaginationParameters(this ControllerBase controller, out int? skip, out int? limit, out bool withCount)
	{
		...
		{
			var withCountString = controller.Request.Query["with_count"];
			withCount = withCountString.ToString().ToLower() == "true";
		}
	}
	...
	{
		controller.ExtractSortingParameters(out var sorting);
	...
	public static void ExtractDateFilterParameters(this ControllerBase controller, out DateTime? startDate, out DateTime? endDate)
	{
		...
		{
			var startDateString = controller.Request.Query["startdate"];
			if (DateTime.TryParseExact(startDateString.ToString(), format, CultureInfo.InvariantCulture, DateTimeStyles.None, out var dateTime))
			{
				startDate = dateTime;
			}
		}
		...
		{
			var endDateString = controller.Request.Query["enddate"];
			if (DateTime.TryParseExact(endDateString.ToString(), format, CultureInfo.InvariantCulture, DateTimeStyles.None, out var dateTime))
			{
				endDate = dateTime;
			}
		}
