...
public class ObjectIdTests
{
    ...
    [Test]
    public void ObjectIdAggregationQuery_Test()
    {
        const string query = "[{\"$match\":{\"sys.created_by\":\"ertugrul.ozcan\"}},{\"$sort\":{\"sys.modified_at\":-1}},{\"$setWindowFields\":{\"sortBy\":{\"sys.modified_at\":-1},\"output\":{\"rank\":{\"$rank\":{}}}}},{\"$match\":{\"_id\":\"68942457bb45a3f8be71e380\"}},{\"$addFields\":{\"pageNumber\":{\"$add\":[{\"$floor\":{\"$divide\":[{\"$subtract\":[\"$rank\",1]},20]}},1]}}}]";
        const string expected = "[{\"$match\":{\"sys.created_by\":\"ertugrul.ozcan\"}},{\"$sort\":{\"sys.modified_at\":-1}},{\"$setWindowFields\":{\"sortBy\":{\"sys.modified_at\":-1},\"output\":{\"rank\":{\"$rank\":{}}}}},{\"$match\":{\"_id\":ObjectId(\"68942457bb45a3f8be71e380\")}},{\"$addFields\":{\"pageNumber\":{\"$add\":[{\"$floor\":{\"$divide\":[{\"$subtract\":[\"$rank\",1]},20]}},1]}}}]";
        var ensured = QueryHelper.EnsureObjectIdsAndISODates(query);
        Assert.That(expected.Replace(" ", string.Empty).Replace("\n", string.Empty).Trim() == ensured.Replace(" ", string.Empty).Replace("\n", string.Empty).Trim());
    }
